...
GaussianInfo<double> SystemSLAM::processNoiseDensity(double dt) const
{
    ...
    double sigma_v = 0.1;  // 20 cm/s per sqrt(second) - increased for better tracking
    
    // Angular velocity noise standard deviation (rad/s/sqrt(s))  
    double sigma_omega = 0.5;  // ~6 degrees/s per sqrt(second) - increased for better tracking
