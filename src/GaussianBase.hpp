...
public:
    ...
    static double chi2inv(double p, double nu)
    {
        assert(p >= 0);
        assert(p < 1);
        // Using equation (32) from the lab document
    ...
    {
        // Using equation (31) from the lab document
    ...
    Eigen::Matrix<Scalar, 2, Eigen::Dynamic> confidenceEllipse(double nSigma = 3.0, int nSamples = 100) const
    {
        ...
        // Get confidence level and chi-squared threshold
        double c = 2.0 * normcdf(nSigma) - 1.0;
        double radius = std::sqrt(chi2inv(c, n));
        
        // Get mean and square root covariance
        Eigen::VectorX<Scalar> mu = mean();
        Eigen::MatrixX<Scalar> S = sqrtCov();
        
        // Generate points on unit circle
        for (int i = 0; i < nSamples; ++i)
        {
            double angle = 2.0 * std::numbers::pi * i / (nSamples - 1);
            Eigen::Vector2<Scalar> w;
            w(0) = radius * std::cos(angle);
            w(1) = radius * std::sin(angle);
            
            // Transform from w-space to x-space: x = mu + S^T * w
            X.col(i) = mu + S.transpose() * w;
        }
